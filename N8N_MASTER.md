...
| **Feedback Review** | Manual Lane = `Feedback Review` → tab **Ready** (needs a real Email). Send workflow drafts Email A with sample + `/go/feedback-review?b=`. Drafts only. | `deploy-outbound-feedback-review-send.sh --setup-tab --activate`; redeploy Manual Lane router |
