...
const SolutionCard: React.FC<{ point: any; index: number }> = ({ point, index }) => {
  ...
  return (
    ...
    >
      <div
        className={`relative w-full h-full transition-all duration-300 bg-white ${isMobile ? "shadow-[8px_8px_0px_0px_#8B6914] -translate-y-2 -translate-x-2" : "group-hover:shadow-[8px_8px_0px_0px_#8B6914] group-hover:-translate-y-2 group-hover:-translate-x-2"}`}
        style={{ transformStyle: "preserve-3d" }}
      >
        <div className="relative bg-white border border-black/10 p-8 md:p-10 flex flex-col justify-start h-full min-h-[380px] md:min-h-[400px]" style={{ backfaceVisibility: "hidden" }}>
          <div className="mb-6 pb-4 border-b border-dark/10">
            <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-gold-on-cream font-bold">0{index + 1}</span>
          </div>
...
const Pillar6: React.FC<Pillar6Props> = ({ onNavigate }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="min-h-screen bg-cream text-dark px-0 relative z-[150] overflow-x-hidden flex flex-col font-sans">
      ...
      <FAQSection faqs={pillarFAQs} accentColor={colors.goldOnCream} title="Questions about training?" subtitle="Common questions about team adoption and onboarding." onNavigate={onNavigate} />
    </motion.div>
  );
};
