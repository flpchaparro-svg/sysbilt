...
const AnimatedCardLeft = ({ data }: { data: any }) => {
  ...
  return (
    <div ref={ref} className="relative w-full flex justify-center items-center py-12 md:min-h-[25vh] md:py-8 overflow-hidden">
      <motion.div style={{ x: lineX, opacity }} className="absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 h-[2px] bg-dark w-[60vw] md:w-[800px] z-0 pointer-events-none" />
      <motion.div style={{ x: cardX, opacity }} className="relative z-10 w-[85vw] md:w-[550px] shrink-0 bg-cream/80 backdrop-blur-md border border-dark p-8 md:p-12">
        <div className="font-mono text-xs font-bold text-dark/60 mb-8 border-b border-dark/10 pb-4 flex justify-end items-center">
          <span className="bg-dark/10 text-dark/70 px-2 py-1">{data.tag || '[SYS_BLEED]'}</span>
        </div>
...
const AnimatedCardRight = ({ data }: { data: any }) => {
  ...
  return (
    <div ref={ref} className="relative w-full flex justify-center items-center py-12 md:min-h-[25vh] md:py-8 overflow-hidden">
      ...
      <motion.div style={{ x: cardX, opacity }} className="relative z-10 w-[85vw] md:w-[550px] shrink-0 bg-cream/80 backdrop-blur-md border border-dark p-8 md:p-12">
        <div className="font-mono text-xs font-bold text-dark/60 mb-8 border-b border-dark/10 pb-4 flex justify-end items-center">
          <span className="bg-dark/10 text-dark/70 px-2 py-1">{data.tag || '[CRITICAL_FAIL]'}</span>
        </div>
        <h3 className="font-serif text-3xl md:text-4xl leading-tight text-dark mb-6">{data.title}</h3>
        <p className="font-sans text-lg text-dark/70 leading-relaxed">{data.desc}</p>
      </motion.div>
...
const SolutionCard: React.FC<{ point: any; index: number }> = ({ point, index }) => {
  ...
  const desktopVariants = {
    ...
    visible: { rotateY: 0, opacity: 1, transition: { duration: 0.8, ease: [0.25, 0.46, 0.45, 0.94], delay: index * 0.15 } }
  };
  ...
  return (
    <motion.div
      ...
      style={{ transformStyle: "preserve-3d", ...(isMobile ? { x: mobileX, opacity: mobileOpacity } : {}) }}
      className="relative w-full h-auto min-h-[400px] md:min-h-[420px] group cursor-pointer"
    >
      <div
        className={`relative w-full h-full transition-all duration-300 bg-white ${isMobile ? "shadow-[8px_8px_0px_0px_#1a1a1a] -translate-y-2 -translate-x-2" : "group-hover:shadow-[8px_8px_0px_0px_#1a1a1a] group-hover:-translate-y-2 group-hover:-translate-x-2"}`}
        style={{ transformStyle: "preserve-3d" }}
      >
        <div className="relative bg-white border border-black/10 p-8 md:p-10 flex flex-col justify-start h-full min-h-[400px] md:min-h-[420px]" style={{ backfaceVisibility: "hidden" }}>
          <div className="mb-6 pb-4 border-b border-dark/10">
            <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-dark font-bold">0{index + 1}</span>
          </div>
          <div className="relative z-10">
            <h3 className="font-serif text-2xl md:text-3xl text-dark mb-4 leading-tight">{point.title}</h3>
...
const Pillar7: React.FC<Pillar7Props> = ({ onNavigate }) => {
  const pillarFAQs = getPillarFAQs('pillar7');
  const { hero, gap, solution, engine } = pillar7Copy;
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="min-h-screen bg-cream text-dark px-0 relative z-[150] overflow-x-hidden flex flex-col font-sans">
      ...
      <section id="engine" className="w-full px-6 md:px-12 lg:px-20 pb-24 max-w-[1400px] mx-auto border-t border-dark/10">
        <div className="pt-16 border-t border-dark/10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            ...
            {/* 3D ORBITAL — P7 Dashboards center (dark), P1+P2+P3 orbiting */}
            <div className="relative aspect-square max-w-[320px] sm:max-w-md mx-auto w-full flex items-center justify-center bg-transparent scale-[0.85] sm:scale-100 origin-center -mt-4 sm:mt-8 lg:mt-0 -mb-8 sm:mb-0" style={{ perspective: "1200px" }}>
              <div className="relative w-full h-full flex items-center justify-center" style={{ transform: "rotateX(70deg)", transformStyle: "preserve-3d" }}>
                <div className="absolute w-48 h-48 rounded-full bg-dark text-cream flex flex-col items-center justify-center shadow-[0_0_40px_rgba(26,26,26,0.4)]" style={{ transform: "rotateX(-70deg)", transformStyle: "preserve-3d" }}>
                  <span className="font-mono text-[10px] tracking-widest opacity-80 mb-1 uppercase">Pillar 7</span>
                  <span className="font-serif font-bold text-lg text-center leading-tight uppercase">Dash-<br/>boards</span>
                </div>
      ...
      <FAQSection faqs={pillarFAQs} accentColor={colors.goldOnCream} title={<>Questions about <span className="italic text-gold-on-cream">dashboards</span></>} subtitle="Common questions about reporting and business intelligence." onNavigate={onNavigate} />
    </motion.div>
  );
};
