#!/usr/bin/env node
/** Writes chatgpt-blog-post-body.json for Sanity patch. */
...
const bullet = (strong, rest) => {
  const children = rest ? [span(strong, ['strong']), span(rest)] : [span(strong, ['strong'])];
  return block('normal', children, { listItem: 'bullet', level: 1 });
};
const imagePlaceholder = (alt, caption, aspect) => ({
  _key: key(),
  _type: 'callout',
  type: 'info',
...
const body = [
  ...
  p('This is the part that matters most to us, because connecting your tools is what we do. On its own, ChatGPT is clever but cut off from your business. Hooked into your systems, it actually knows it.'),
  ...
  p("Two honest catches. Those direct connections sit behind a paid plan, not the free one, and the list of what connects changes month to month, so check before you count on it. Some countries can't switch them on yet, and anything touching customer or money data needs setting up properly, with a clear eye on where that data ends up."),
  ...
  h2('How we do it'),
  p('We start clients on the free chat to prove it earns its place on real work, then build the jobs that repeat into the background with sensible limits, so it does the boring work without the runaway bill or the bad habits.'),
];

writeFileSync(resolve(import.meta.dirname, 'chatgpt-blog-post-body.json'), JSON.stringify(body, null, 2));
