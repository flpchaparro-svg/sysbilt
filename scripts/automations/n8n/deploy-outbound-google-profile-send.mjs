#!/usr/bin/env node
/**
 * Deploy SYSBILT - Outbound Google Profile Send.
 *
 * Google Profile Status=Ready + real Email → Gmail draft + /go/google-profile?b=
 * → Status=Emailed. Drafts only.
 *
 * Usage:
 *   node scripts/automations/n8n/deploy-outbound-google-profile-send.mjs
 *   node scripts/automations/n8n/deploy-outbound-google-profile-send.mjs --activate
 */
...
const GP_RANGE = 'A1:I5000';
const LEADS_RANGE = 'A1:O5000';
const FUNNEL_BASE = 'https://sysbilt.com/go/google-profile';
...
const reviews = String(row.Reviews || lead.Reviews || '').trim() || 'few';
...
const subject = business + "'s Google profile is thin";

const place = suburb ? ' in ' + esc(suburb) : '';
  ...
  const nodes = [
    ...
    },
    {
      ...
      parameters: {
        ...
        options: {
          dataLocationOnSheet: {
            values: { rangeDefinition: 'specifyRangeA1', range: GP_RANGE },
          },
        },
      },
    },
    ...
    },
    {
      ...
      parameters: {
        ...
        height: 360,
        color: 5,
        content: `## Outbound Google Profile Send

**What it does**
Status = **Ready** + real Email → Gmail **draft** + /go/google-profile?b= → **Emailed**.

**Does not send.** Review Drafts, then Send.

**Status dropdown**
Ready · Wait · Emailed · Replied · Dead

Flip **Wait → Ready** when product 1 went quiet and you want this offer next.`,
      },
    },
  ];

  const connections = {
    'Every 5 Minutes': { main: [[{ node: 'Read Google Profile Tab', type: 'main', index: 0 }]] },
    'Manual Trigger': { main: [[{ node: 'Read Google Profile Tab', type: 'main', index: 0 }]] },
    'Read Google Profile Tab': { main: [[{ node: 'Pick Ready Row', type: 'main', index: 0 }]] },
  ...
  return {
    name: 'SYSBILT - Outbound Google Profile Send',
    nodes,
    connections,
...
async function main() {
  ...
  saveDeployState({ OUTBOUND_GOOGLE_PROFILE_SEND_WORKFLOW_ID: wf.id });

  console.log(`\nGoogle Profile Send${activate ? ' (active)' : ' (inactive)'}: ${N8N_BASE}/workflow/${wf.id}`);
  console.log('Flow: Google Profile Status=Ready → Gmail draft → Emailed');
}
