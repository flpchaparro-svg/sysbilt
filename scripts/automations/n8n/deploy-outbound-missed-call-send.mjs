#!/usr/bin/env node
/**
 * Deploy SYSBILT - Outbound Missed-Call Send.
 *
 * Missed-Call Status=Ready + real Email → Gmail draft + /go/missed-call?b=
 * → Status=Emailed. Drafts only.
 *
 * Usage:
 *   node scripts/automations/n8n/deploy-outbound-missed-call-send.mjs
 *   node scripts/automations/n8n/deploy-outbound-missed-call-send.mjs --activate
 */
...
const MC_RANGE = 'A1:I5000';
const LEADS_RANGE = 'A1:O5000';
const FUNNEL_BASE = 'https://sysbilt.com/go/missed-call';
...
const subject = 'What happens when ' + business + ' misses a call';
  ...
  const nodes = [
    ...
    },
    {
      ...
      parameters: {
        ...
        options: {
          dataLocationOnSheet: {
            values: { rangeDefinition: 'specifyRangeA1', range: MC_RANGE },
          },
        },
      },
    },
    ...
    },
    {
      ...
      parameters: {
        ...
        height: 360,
        color: 5,
        content: `## Outbound Missed-Call Send

**What it does**
Status = **Ready** + Phone + Email → Gmail **draft** + /go/missed-call?b= → **Emailed**.
...
Ready · Wait · Emailed · Replied · Dead`,
  ...
  const connections = {
    'Every 5 Minutes': { main: [[{ node: 'Read Missed-Call Tab', type: 'main', index: 0 }]] },
    'Manual Trigger': { main: [[{ node: 'Read Missed-Call Tab', type: 'main', index: 0 }]] },
    'Read Missed-Call Tab': { main: [[{ node: 'Pick Ready Row', type: 'main', index: 0 }]] },
  ...
  return {
    name: 'SYSBILT - Outbound Missed-Call Send',
    nodes,
    connections,
...
async function main() {
  ...
  saveDeployState({ OUTBOUND_MISSED_CALL_SEND_WORKFLOW_ID: wf.id });

  console.log(`\nMissed-Call Send${activate ? ' (active)' : ' (inactive)'}: ${N8N_BASE}/workflow/${wf.id}`);
  console.log('Flow: Missed-Call Status=Ready → Gmail draft → Emailed');
}
