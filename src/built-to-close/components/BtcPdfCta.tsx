...
import { BTC_BOOK_PATH, BTC_HUB_PATH } from '../chapter-seo'
...
/** Points readers to the gated A4 book / print experience. */
export function BtcPdfCta({ variant = 'card', className = '' }: Props) {
  if (variant === 'inline') {
    return (
      <p className={`font-sans text-[15px] leading-relaxed text-dark/70 ${className}`}>
        ...
        >
          Download the polished PDF edition of {BTC_META.title}
        </Link>
        .
      </p>
    )
