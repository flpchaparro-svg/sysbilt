...
export const ch06Pages: BtwPage[] = [
  ...
  flow(
    ...
    ...section(
      ...
      ...p('The day starts with the CRM, not the inbox, and it takes about fifteen minutes.'),
      ...p('First, the new leads: everything that arrived since yesterday, each one answered or owned before anything else happens, because chapter one\'s arithmetic has not changed overnight, the fresh enquiry is the most perishable thing the business holds. Second, today\'s tasks: the follow-ups, the promised calls, the quotes due a nudge, worked as the list the system wrote from your own commitments. Third, a glance at the board: anything sitting past its stage rhythm gets a decision, act, or move it honestly.'),
      ...p('That is the whole ceremony. Its power is not any single morning but the compounding: a business that starts every day this way simply does not leak the way chapter one described, and the owner who used to carry the follow-up in their head now carries nothing, because the list is the memory.'),
      {
        type: 'diagram',
        id: 'btc-ch06-fifteen-minutes',
        caption: 'The fifteen minutes. 1 NEW LEADS answered or owned first. 2 TODAY\'S TASKS the list the system wrote. 3 THE BOARD anything sitting past rhythm gets a decision.',
      },
    ),
  ),
  flow(
    ...section(
      'INBOX ZERO',
      'Lead-inbox zero',
      ...p('One reflex deserves its own name. Every lead, from every source, reaches a state of answered-and-owned the same day it arrives: replied to, assigned, next step set. Not necessarily closed, not even quoted, just never floating. The unified inbox from chapter five makes this mechanical, and the speed-to-lead wiring makes the first touch instant even when you are on a job. The habit is refusing to let anything sit unowned overnight, because unowned is where leads go to die, and the whole system exists so they do not.'),
    ),
  ),
  flow(
    ...section(
  ...
  flow(
    ...section(
      ...
      ...p('At some point something will look wrong, a customer says they enquired and nothing is in the system, or a sequence seems to have gone quiet. Before panic, the drill.'),
      {
        type: 'fireDrill',
        title: 'Lead fire drill',
        steps: [
