...
export const ch07Pages: BtwPage[] = [
  opener({
    ...
    subtitle: 'The craft of cadence, content, stopping, and the machinery underneath.',
  }),
  ...
  flow(
    ...section(
      ...
      'Persistent, or pest',
      ...p(
        'The line between the two is not frequency. It is value. A pest\'s message asks for something and offers nothing: just checking in, just following up, any thoughts. Each one says, in effect, do my admin for me. A persistent professional\'s message gives before it asks: the answer to a doubt, a useful detail, a relevant example, a genuine deadline, and then makes the next step easy. Same cadence, opposite experience, because one is pressure and the other is service.',
        'So the standing rule for every sequence written in this system: every touch carries something. If a message cannot offer anything, information, help, ease, honesty, it is not ready to send, and just checking in is banned from the building. This single rule is most of the craft; everything else is timing.',
      ),
      {
        type: 'diagram',
        id: 'btc-ch07-persistent-pest',
        caption:
          'Persistent, or pest. LEFT (faded): "just checking in", "any thoughts?", "bumping this", tagged "asks, offers nothing". RIGHT (gold accent): "the doubt, answered", "a useful detail", "an honest timing note", tagged "gives before it asks". Footer: "Same cadence. Opposite experience."',
      },
    ),
  ),
  ...
  flow(
    ...section(
      ...
      {
        type: 'diagram',
        id: 'btc-ch07-cadence-shape',
        caption:
          'The shape of a follow-up. A decaying-rhythm timeline: touches close together early, breathing longer later, mixed channel icons (email, phone, message) on the touches, ending in a labelled "graceful close". One line beneath: "Front-loaded, decaying, every touch worth receiving."',
      },
    ),
  ),
  ...
  flow(
    ...section(
      ...
      'The graceful close',
      ...p(
        'Every sequence ends, and the ending is a tool, not a defeat. The close-out message says, kindly and plainly: we will stop chasing, the door stays open, here is how to reach us when the timing is right. It works twice over. It earns replies, the honesty of it regularly shakes loose a decision that pressure never would, and it protects the relationship, because the person who went quiet in April may be ready in October, and how you stopped is what they will remember. The closed deal goes to lost with its reason, the contact stays warm on the long-cycle list, and chapter five\'s win-back machinery takes it from there. Stopping well is a skill, and businesses that master it get to follow up harder in the middle, because the exit is guaranteed to be graceful.',
      ),
    ),
  ),
  ...
  flow(
    ...
    }),
    {
      ...
      caption:
        'The reputation you cannot see. A domain node with a hidden meter beneath it labelled "sender reputation". Two flows out: HIGH → inbox icons, "your messages arrive". DAMAGED → junk-folder icon, "sent, but nobody sees, no error shown". Feeding the meter, two small arrows: "relevance and hygiene" raising it, "spam-button clicks" draining it.',
    },
  ),
