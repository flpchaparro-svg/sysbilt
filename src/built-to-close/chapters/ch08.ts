...
export const ch08Pages: BtwPage[] = [
  opener({
    ...
    subtitle: 'Capture at birth, read with honesty, decide quarterly.',
  }),
  ...
  flow(
    ...
    }),
    {
      ...
      caption:
        'The journey and the dark funnel. A buyer\'s path: coffee-table recommendation (dashed, unmeasurable) → social post seen (dashed) → searched your name (solid) → website form (solid, tagged). The solid final step wearing a crown labelled "gets all the credit", the dashed early steps labelled "the dark funnel: real, invisible". Footer: "Evidence, not gospel."',
    },
  ),
  ...
  flow(
    ...section(
      ...
      ...p(
        'Tracking earns nothing until it moves money, so the loop closes in a standing quarterly decision: leads, won deals and won revenue by source, read side by side, and the budget and effort shifted toward what demonstrably pays. The patterns that emerge are almost always more useful than the totals: the channel that brings many leads that rarely close, a message-to-market mismatch worth fixing before worth cutting; the source whose leads close big but slowly; the referrals quietly outperforming everything, which is not a coincidence but an instruction, build the review-and-referral machinery harder. And one number worth knowing per channel even roughly: what a won customer costs from each source, because it turns the eternal should we spend more on this from a feeling into arithmetic.',
        'That is the whole discipline: capture at birth, read with honesty, decide quarterly. The machine that makes it effortless, the wiring that moves every lead, message and dollar between systems without human ferrying, is the next chapter, where the CRM takes its place at the centre of the business.',
      ),
    ),
  ),
]
