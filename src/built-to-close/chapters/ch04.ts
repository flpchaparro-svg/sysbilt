...
export const ch04Pages: BtwPage[] = [
  ...
  flow(
    ...
    ...section(
      ...
      ...p('The instinct when setting up a CRM is to add a field for everything you might ever want to know. The instinct is wrong, and it is the number one way setups sabotage themselves. Every field is a small tax on every person every time they touch a record, and fields that are taxing get skipped, and skipped fields breed records that are half-true, which is worse than absent, because half-true gets trusted.'),
      ...p('The rule: a field earns its place only if someone will act differently because of what it holds. Name, contact details, source, owner, and the handful of facts your actual sales conversation turns on. That is usually under a dozen. Everything else is a note, notes are free-form and honest, and the record stays quick enough to update from a phone in sixty seconds, which is the real adoption test.'),
      {
        type: 'diagram',
        id: 'btc-ch04-field-earns',
        caption: 'A field earns its place. KEEP: name, contact, source, owner, the facts the sale turns on. EVERYTHING ELSE: a note. Under a minute from a phone, or the setup has failed.',
      },
    ),
  ),
  flow(
    ...section(
      ...
      ...p('One field is sacred, and it gets its own commandment: every contact carries where it came from. Website form, phone call, referral, social, walk-in. Tagged at the moment of capture, automatically wherever the wiring allows, because memory-based source data is fiction by Friday. This single discipline is what makes chapter eight possible at all, knowing which marketing actually pays, and it cannot be reconstructed later. Start it on day one even if you start nothing else.'),
    ),
  ),
  flow(
    ...section(
      ...
      ...p('Chapter one called the CRM the memory of the business, and a memory that holds the same person as three people is a memory with amnesia. Duplicates are how follow-up emails the same client twice in a morning, how the history of a relationship scatters into fragments too thin to matter, and how your best customer hides inside three mediocre records. The defences: capture through wired forms rather than retyping, let the system\'s own duplicate-catching do its work, and merge on sight as a reflex, not a project. One human, one record, is the rule everything else leans on.'),
      {
        type: 'diagram',
        id: 'btc-ch04-one-human',
        caption: 'One human, one record. Three thin duplicate records merging into one full record with history and value.',
      },
    ),
  ),
  flow(
    ...section(
      ...
      ...p('Data rots at rest. People change jobs, numbers, and minds, and a contact record untouched for two years is a guess wearing a name tag. The answer is not a heroic annual cleanse, heroic cleanses get scheduled and skipped, it is a small standing rhythm: merge duplicates as they appear, retire the truly dead rather than letting them pad the numbers, and correct details in the moment you learn they changed, which takes ten seconds now and an archaeology later. A monthly half hour on the worst of it, and the memory stays trustworthy.'),
      ...p('Because that is the whole stake, trust. A team that opens the CRM and finds it true, current and quick starts treating it as the single source of truth, and everything in the chapters ahead compounds from there. A clean small CRM beats a bloated big one every single time, and it is not close. Now, onto the machinery that feeds it: the features.'),
      {
        type: 'diagram',
        id: 'btc-ch04-hygiene-rhythm',
        caption: 'The hygiene rhythm. IN THE MOMENT: correct details as you learn them. ON SIGHT: merge duplicates as a reflex. MONTHLY: half an hour on the worst of it.',
      },
    ),
  ),
]
