...
export const ch09Pages: BtwPage[] = [
  ...
  flow(
    ...section(
      ...
      {
        type: 'diagram',
        id: 'btc-ch09-sent-received',
        caption: 'Sent is not received. TOP: send and hope. BOTTOM: confirm, queue, retry. The gap between sent and received is where leads vanish.',
      },
    ),
  ),
  flow(
  ...
  flow(
    ...section(
      ...
      ...p('A closing note that echoes every book in this series, because here it has teeth. The hub\'s value is the memory it accumulates, and that memory must live on ground you own: your account, your export rights, your documented configuration. A decade of relationship history is the least replaceable asset the business will ever build. Wire it deep, and hold the keys.'),
    ),
  ),
  flow(
    ...section(
      'IN SHORT',
      'In short',
      ...p('Seen whole, the CRM stops being software and becomes what chapter one promised: the memory of the business, awake, connected, and acting, catching what the website sends, remembering what every conversation adds, chasing what would have gone quiet, and telling the money systems the moment there is money to handle. What remains is the human layer that makes or breaks all of it, the team actually using the thing, and that is chapter ten.'),
    ),
  ),
]
