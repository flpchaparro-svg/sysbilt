...
import { BTR_HUB_OG } from '../built-to-run/chapter-covers'

export default function BuiltToRunHubPage() {
  const hubUrl = `${SITE_ORIGIN}${BTR_HUB_PATH}`
  const htmlTitle = 'Business Automation That Works: The Complete Guide | SYSBILT'
  const description = BTR_META.seoDescription
    ...
    <div className="min-h-screen bg-cream text-dark selection:bg-dark selection:text-cream pt-[100px] md:pt-[140px] pb-16 md:pb-24">
      ...
      <div className="mx-auto w-full max-w-[840px] px-4 md:px-6">
        <nav aria-label="Breadcrumb" className="mb-8 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-dark/45">
          ...
          <span className="mx-2">/</span>
          <span className="text-dark/70">{BTR_META.title}</span>
        </nav>
        ...
        <header className="mb-12 md:mb-16 border-b-2 border-dark pb-10">
          ...
          <h1 className="font-serif text-[clamp(40px,7vw,64px)] font-medium leading-[1.02] tracking-[-0.035em] text-dark m-0 mb-5">
            How automation really works now
          </h1>
          <p className="font-sans text-[17px] md:text-[19px] leading-relaxed text-dark/70 max-w-prose m-0">
            {BTR_META.subtitle} Read every chapter free on the web, or download the full A4 edition to print or save as PDF.
          </p>
        </header>

        <section aria-labelledby="btr-chapters-heading" className="mb-14">
          <h2
            id="btr-chapters-heading"
