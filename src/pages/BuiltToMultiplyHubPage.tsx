...
import { BTM_CHAPTERS, BTM_HUB_PATH, btmChapterPath } from '../built-to-multiply/chapter-seo'
import { BtmPdfCta } from '../built-to-multiply/components/BtmPdfCta'
import { BTM_META } from '../built-to-multiply/types'
import { BTM_HUB_OG } from '../built-to-multiply/chapter-covers'

export default function BuiltToMultiplyHubPage() {
  const hubUrl = `${SITE_ORIGIN}${BTM_HUB_PATH}`
  const htmlTitle = 'Built to Multiply: Content Without the Flood | SYSBILT'
    ...
    <div className="min-h-screen bg-cream text-dark selection:bg-dark selection:text-cream pt-[100px] md:pt-[140px] pb-16 md:pb-24">
      ...
      <div className="mx-auto w-full max-w-[840px] px-4 md:px-6">
        ...
        <header className="mb-12 md:mb-16 border-b-2 border-dark pb-10">
          <div className="font-mono text-[11px] font-bold uppercase tracking-[0.24em] text-gold-on-cream mb-4">/ A field guide for content</div>
          <h1 className="font-serif text-[clamp(40px,7vw,64px)] font-medium leading-[1.02] tracking-[-0.035em] text-dark m-0 mb-5">
            How one voice becomes a month of content
          </h1>
          <p className="font-sans text-[17px] md:text-[19px] leading-relaxed text-dark/70 max-w-prose m-0">
            {BTM_META.subtitle} Read every chapter free on the web, or download the full A4 edition to print or save as PDF.
          </p>
        </header>
        <section aria-labelledby="btm-chapters-heading" className="mb-14">
          <h2 id="btm-chapters-heading" className="font-mono text-[10px] font-bold uppercase tracking-[0.22em] text-dark/45 mb-6 m-0">Twelve chapters</h2>
          <ol className="list-none p-0 m-0 flex flex-col gap-0 border-2 border-dark divide-y divide-dark">
            {BTM_CHAPTERS.map((ch) => (
              <li key={ch.slug}>
                <Link to={btmChapterPath(ch.slug)} className="group flex items-start gap-4 md:gap-6 p-5 md:p-6 hover:bg-dark hover:text-cream transition-colors">
