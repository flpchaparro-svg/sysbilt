...
import {
  ...
  getBteChapterByNum,
  pillarLabel,
} from '../built-to-teach/chapter-seo'
import { BTE_CHAPTER_COVERS } from '../built-to-teach/chapter-covers'
...
export default function BteChapterArticlePage() {
  const { chapterSlug } = useParams<{ chapterSlug: string }>()
  const chapter = chapterSlug ? getBteChapterBySlug(chapterSlug) : undefined

  if (!chapter) {
    return <Navigate to={BTE_HUB_PATH} replace />
  }

  const blocks = extractChapterBlocks(chapter.pages)
  const cover = BTE_CHAPTER_COVERS[chapter.num]
  const canonical = `${SITE_ORIGIN}${bteChapterPath(chapter.slug)}`
  const htmlTitle = `${chapter.seoTitle} | SYSBILT`
  const prev = chapter.num > 1 ? getBteChapterByNum(chapter.num - 1) : undefined
  const next = chapter.num < BTE_CHAPTERS.length ? getBteChapterByNum(chapter.num + 1) : undefined
    ...
    <div className="btw-root min-h-screen bg-cream text-dark selection:bg-dark selection:text-cream pt-[100px] md:pt-[140px] pb-16 md:pb-24">
      ...
      <article className="mx-auto w-full max-w-[720px] px-4 md:px-6">
        <nav aria-label="Breadcrumb" className="mb-8 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-dark/45">
          ...
          <Link to={BTE_HUB_PATH} className="hover:text-dark transition-colors">
            {BTE_META.title}
          </Link>
          <span className="mx-2">/</span>
        ...
        <GuideChapterNav prev={prev} next={next} chapterPath={bteChapterPath} />
...
export function isBteChapterSlug(slug: string): boolean {
  if (slug === 'read') return false
  return BTE_CHAPTERS.some((c) => c.slug === slug)
}
