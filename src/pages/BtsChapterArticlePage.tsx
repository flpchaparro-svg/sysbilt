...
export default function BtsChapterArticlePage() {
  const { chapterSlug } = useParams<{ chapterSlug: string }>()
  const chapter = chapterSlug ? getBtsChapterBySlug(chapterSlug) : undefined

  if (!chapter) {
    return <Navigate to={BTS_HUB_PATH} replace />
  }

  const blocks = extractChapterBlocks(chapter.pages)
  const cover = BTS_CHAPTER_COVERS[chapter.num]
  const canonical = `${SITE_ORIGIN}${btsChapterPath(chapter.slug)}`
  const htmlTitle = `${chapter.seoTitle} | SYSBILT`
  const prev = chapter.num > 1 ? getBtsChapterByNum(chapter.num - 1) : undefined
  const next = chapter.num < BTS_CHAPTERS.length ? getBtsChapterByNum(chapter.num + 1) : undefined
    ...
    <div className="btw-root min-h-screen bg-cream text-dark selection:bg-dark selection:text-cream pt-[100px] md:pt-[140px] pb-16 md:pb-24">
      ...
      <article className="mx-auto w-full max-w-[720px] px-4 md:px-6">
        <nav aria-label="Breadcrumb" className="mb-8 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-dark/45">
          ...
          <Link to={BTS_HUB_PATH} className="hover:text-dark transition-colors">
            {BTS_META.title}
          </Link>
          <span className="mx-2">/</span>
        ...
        <GuideChapterNav prev={prev} next={next} chapterPath={btsChapterPath} />
...
export function isBtsChapterSlug(slug: string): boolean {
  if (slug === 'read') return false
  return BTS_CHAPTERS.some((c) => c.slug === slug)
}
