...
import { BSE_HUB_PATH, BSE_BOOK_PATH } from '../built-to-see/chapter-seo'
import { BSE_CHAPTER_COVERS, BSE_HUB_OG } from '../built-to-see/chapter-covers'
import { BseCoverPage, BseClosingPage } from '../built-to-see/components/BseCover'
import { BSE_CONTENT_PAGES } from '../built-to-see/pages'
import { BSE_META } from '../built-to-see/types'

const BSE_GUIDE_BOOK = {
  runningHeadTitle: BSE_META.title,
  chapterCovers: BSE_CHAPTER_COVERS,
} as const

/** Gated A4 book — noindex; public chapters are the canonical SEO surface. */
export default function BuiltToSeeBookPage() {
  const [isUnlocked, setIsUnlocked] = useState(() => {
    if (typeof window === 'undefined') return false
    return localStorage.getItem('sysbilt_known_user') === 'true'
  })

  const bookUrl = `${SITE_ORIGIN}${BSE_BOOK_PATH}`
  ...
  return (
    <div className="btw-root btw-book-shell min-h-screen bg-[#1A1A1A] selection:bg-[#C5A059] selection:text-[#1A1A1A] pt-[100px] md:pt-[140px] pb-16 md:pb-24 print:pt-0 print:pb-0">
      ...
      <PageMeta
        title={`${BSE_META.title} — Full edition | SYSBILT`}
        description="Download or print the full A4 edition of Built to See."
        canonical={bookUrl}
        robots="noindex, follow"
        ogImage={`${SITE_ORIGIN}${BSE_HUB_OG}`}
      />
      <Helmet>
      ...
      <div className="btw-page-stack mx-auto flex w-full max-w-[840px] flex-col items-center gap-10 md:gap-14 px-4">
        ...
        {!isUnlocked ? (
          <div className="w-full max-w-[794px] relative z-20 print:hidden">
            <GuideGateForm
              guideSlug={BSE_META.slug}
              guideName={BSE_META.title}
        ...
        {isUnlocked ? (
          <>
            <BtwMeasuredGuide rawPages={BSE_CONTENT_PAGES} guide={BSE_GUIDE_BOOK} />
            <BseClosingPage />
          </>
        ) : null}
      </div>
