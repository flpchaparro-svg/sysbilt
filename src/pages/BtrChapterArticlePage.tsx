...
import {
  ...
  getBtrChapterByNum,
  pillarLabel,
} from '../built-to-run/chapter-seo'
import { BTR_CHAPTER_COVERS } from '../built-to-run/chapter-covers'
import { chapterCoverWebSrc } from '../guides/chapter-cover-types'
import { BtwFlowList } from '../built-to-work/components/BtwBlocks'
import { BtrPdfCta } from '../built-to-run/components/BtrPdfCta'
...
export default function BtrChapterArticlePage() {
  const { chapterSlug } = useParams<{ chapterSlug: string }>()
  const chapter = chapterSlug ? getBtrChapterBySlug(chapterSlug) : undefined

  if (!chapter) {
    return <Navigate to={BTR_HUB_PATH} replace />
  }

  const blocks = extractChapterBlocks(chapter.pages)
  const cover = BTR_CHAPTER_COVERS[chapter.num]
  const canonical = `${SITE_ORIGIN}${btrChapterPath(chapter.slug)}`
  const htmlTitle = `${chapter.seoTitle} | SYSBILT`
  const prev = chapter.num > 1 ? getBtrChapterByNum(chapter.num - 1) : undefined
  const next = chapter.num < BTR_CHAPTERS.length ? getBtrChapterByNum(chapter.num + 1) : undefined
    ...
    <div className="btw-root min-h-screen bg-cream text-dark selection:bg-dark selection:text-cream pt-[100px] md:pt-[140px] pb-16 md:pb-24">
      ...
      <article className="mx-auto w-full max-w-[720px] px-4 md:px-6">
        <nav aria-label="Breadcrumb" className="mb-8 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-dark/45">
          ...
          <Link to={BTR_HUB_PATH} className="hover:text-dark transition-colors">
            {BTR_META.title}
          </Link>
          <span className="mx-2">/</span>
        ...
        <GuideChapterNav prev={prev} next={next} chapterPath={btrChapterPath} />
...
export function isBtrChapterSlug(slug: string): boolean {
  if (slug === 'read') return false
  return BTR_CHAPTERS.some((c) => c.slug === slug)
}
