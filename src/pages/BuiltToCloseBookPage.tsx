...
import { BTC_HUB_PATH, BTC_BOOK_PATH } from '../built-to-close/chapter-seo'
import { BTC_CHAPTER_COVERS, BTC_HUB_OG } from '../built-to-close/chapter-covers'
import { BtcCoverPage, BtcClosingPage } from '../built-to-close/components/BtcCover'
import { BTC_CONTENT_PAGES } from '../built-to-close/pages'
import { BTC_META } from '../built-to-close/types'

const BTC_GUIDE_BOOK = {
  runningHeadTitle: BTC_META.title,
  chapterCovers: BTC_CHAPTER_COVERS,
} as const

/** Gated A4 book — noindex; public chapters are the canonical SEO surface. */
export default function BuiltToCloseBookPage() {
  const [isUnlocked, setIsUnlocked] = useState(() => {
    if (typeof window === 'undefined') return false
    return localStorage.getItem('sysbilt_known_user') === 'true'
  })

  const bookUrl = `${SITE_ORIGIN}${BTC_BOOK_PATH}`
  ...
  return (
    <div className="btw-root min-h-screen bg-[#1A1A1A] selection:bg-[#C5A059] selection:text-[#1A1A1A] pt-[100px] md:pt-[140px] pb-16 md:pb-24 print:pt-0 print:pb-0">
      ...
      <PageMeta
        title={`${BTC_META.title} — Full edition | SYSBILT`}
        description="Download or print the full A4 edition of Built to Close."
        canonical={bookUrl}
        robots="noindex, follow"
        ogImage={`${SITE_ORIGIN}${BTC_HUB_OG}`}
      />
      <Helmet>
      ...
      <div className="btw-page-stack mx-auto flex w-full max-w-[840px] flex-col items-center gap-10 md:gap-14 px-4">
        ...
        {!isUnlocked ? (
          <div className="w-full max-w-[794px] relative z-20 print:hidden">
            <GuideGateForm
              guideSlug={BTC_META.slug}
              guideName={BTC_META.title}
        ...
        {isUnlocked ? (
          <>
            <BtwMeasuredGuide rawPages={BTC_CONTENT_PAGES} guide={BTC_GUIDE_BOOK} />
            <BtcClosingPage />
          </>
        ) : null}
      </div>
