...
/** Leak: hobby inbox beside a DM desk with quick replies. */
export function DmReplyLeakPair() {
    ...
    >
      ...
      >
        ...
        <div className="p-4 md:p-5 flex flex-col justify-center gap-2 min-h-[168px]">
          {['One person has the login', 'Replies at midnight or never', 'Ads dump into silence'].map((item) => (
            <div
              key={item}
      ...
      >
        ...
        <div className="p-4 md:p-5 flex flex-col justify-center gap-2 min-h-[168px]">
          {['Quick replies live', 'Handoff to a human', 'Ownership documented'].map((item, i) => (
            <motion.div
              key={item}
