...
export function DmReplyPainCards() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
      <Card index="01" title="Only one person has the login">
        {({play, reduce}) => (
          <div className="space-y-2 py-1">
            {['Instagram', 'Facebook Page', 'Ads inbox'].map((ch, i) => (
              <motion.div
                key={ch}
      ...
      <Card index="02" title="Replies go out at midnight">
        {({play}) => (
          <div className="flex flex-col items-center justify-center gap-3 py-2">
            <div className="w-full rounded-lg border px-3 py-3 text-center" style={{borderColor: `${FUNNEL_COLOURS.ink}14`, backgroundColor: '#fff'}}>
              <p className="font-mono text-[20px] font-bold tabular-nums" style={{color: FUNNEL_COLOURS.accent}}>
                00:47
              </p>
              <p className="font-sans text-[12px] mt-1" style={{color: FUNNEL_COLOURS.ink}}>
                Or not at all
              </p>
            </div>
            <motion.span
      ...
      <Card index="03" title="No record of what was promised">
        {({play, reduce}) => (
          <div className="space-y-3 py-1">
            {[
              {label: 'Price quoted', value: 'Lost'},
              {label: 'Next step', value: 'Vague'},
            ].map((row, i) => (
              <div key={row.label}>
                ...
                <div className="h-1.5 rounded-full overflow-hidden" style={{backgroundColor: `${FUNNEL_COLOURS.ink}2C`}}>
                  <motion.div
                    ...
                    animate={play || reduce ? {width: i === 0 ? '28%' : '40%'} : {width: '0%'}}
      ...
      <Card index="04" title="Ads drive DMs into a black hole">
        {({play, reduce}) => (
