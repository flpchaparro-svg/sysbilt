...
export function BundleSpeedNextPainCards() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
      <Card index="01" title="Pages are quick and unclear">
        {({play, reduce}) => (
          <div className="space-y-2 py-1">
            {[
              {label: 'Load', value: 'Fast'},
              {label: 'Ask', value: 'Vague'},
              {label: 'Next step', value: 'Buried'},
            ].map((row, i) => (
              <motion.div
                key={row.label}
              ...
              >
                ...
                <span
                  ...
                  style={{
                    color: row.value === 'Fast' ? FUNNEL_COLOURS.goldDeep : FUNNEL_COLOURS.accent,
                  }}
                >
                  {row.value}
                </span>
      ...
      <Card index="02" title="Nobody knows if enquiries rose">
        {({play}) => (
          <div className="flex flex-col items-center justify-center gap-3 py-2">
            <div className="w-full grid grid-cols-3 gap-1.5">
              {[0, 1, 2].map((i) => (
                <div
                  key={i}
                  className="h-10 rounded-sm flex items-center justify-center"
                  style={{backgroundColor: `${FUNNEL_COLOURS.accent}${i === 1 ? '35' : '18'}`}}
                >
                  <span className="font-mono text-[8px] font-bold" style={{color: FUNNEL_COLOURS.accent}}>
                    ?
                  </span>
                </div>
              ))}
      ...
      <Card index="03" title="Second kickoff feels heavy">
        {({play, reduce}) => (
          <div className="space-y-3 py-1">
            {[
              {label: 'Speed Fix access', value: 'Warm', ok: true},
              {label: 'New vendor start', value: 'Cold', ok: false},
            ].map((row, i) => (
              <div key={row.label}>
                ...
                <div className="h-1.5 rounded-full overflow-hidden" style={{backgroundColor: `${FUNNEL_COLOURS.ink}2C`}}>
                  <motion.div
                    ...
                    animate={play || reduce ? {width: row.ok ? '88%' : '28%'} : {width: '0%'}}
      ...
      <Card index="04" title="Marketing wants proof">
        {({play, reduce}) => (
