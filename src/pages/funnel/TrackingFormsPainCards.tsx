...
export function TrackingFormsPainCards() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
      <Card index="01" title="Which page produced it?">
        {({play, reduce}) => (
          <div className="space-y-2 py-1">
            {['Home', 'Services', 'Contact'].map((page, i) => (
              <motion.div
                key={page}
      ...
      <Card index="02" title="Forms to a dead inbox">
        {({play}) => (
          <div className="flex flex-col items-center justify-center gap-3 py-2">
            ...
            >
              Bounces · Nobody watches
            </motion.span>
          </div>
        )}
      </Card>

      <Card index="03" title="Ads and speed unjudgeable">
        {({play, reduce}) => (
          <div className="space-y-3 py-1">
            {[
              {label: 'Spend', value: 'Up', ok: false},
              {label: 'Enquiries attributed', value: 'None', ok: false},
            ].map((row, i) => (
              <div key={row.label}>
                ...
                <div className="h-1.5 rounded-full overflow-hidden" style={{backgroundColor: `${FUNNEL_COLOURS.ink}2C`}}>
                  <motion.div
                    ...
                    animate={play || reduce ? {width: row.ok ? '78%' : '12%'} : {width: '0%'}}
      ...
      <Card index="04" title="GA4 installed, never checked">
        {({play, reduce}) => (
