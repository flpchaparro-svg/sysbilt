...
export function InboxTriagePainCards() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
      <Card index="01" title="Everything in one stream">
        {({play, reduce}) => (
          <div className="space-y-2 py-1">
            {['Promo', 'Client', 'Vendor', 'FYI'].map((label, i) => (
              <motion.div
                key={label}
      ...
      <Card index="02" title="Same reply, rewritten daily">
        {({play}) => (
      ...
      <Card index="03" title="Urgent mail looks like noise">
        {({play, reduce}) => (
          <div className="space-y-3 py-1">
            {[
              {label: 'Newsletter pile', value: 'Loud', ok: false},
              {label: 'Client reply needed', value: 'Buried', ok: false},
            ].map((row, i) => (
              <div key={row.label}>
                ...
                <div className="h-1.5 rounded-full overflow-hidden" style={{backgroundColor: `${FUNNEL_COLOURS.ink}2C`}}>
                  <motion.div
                    ...
                    animate={play || reduce ? {width: i === 0 ? '92%' : '28%'} : {width: '0%'}}
      ...
      <Card index="04" title="Shared inboxes, no rules">
        {({play, reduce}) => (
          <div className="relative flex flex-col items-center justify-center gap-2 py-2">
            ...
            >
              ...
              <p className="font-sans text-[13px] font-semibold" style={{color: FUNNEL_COLOURS.ink}}>
                Everyone invents their own sort
              </p>
            </motion.div>
            <motion.p
