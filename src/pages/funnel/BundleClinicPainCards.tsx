...
export function BundleClinicPainCards() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
      <Card index="01" title="Profile looks unfinished">
        {({play, reduce}) => (
          <div className="space-y-2 py-1">
            {['Hours wrong', 'Photos thin', 'Services vague'].map((row, i) => (
              <motion.div
                key={row}
      ...
      <Card index="02" title="Nobody asks for reviews">
        {({play}) => (
      ...
      <Card index="03" title="Missed call becomes theirs">
        {({play, reduce}) => (
          <div className="space-y-3 py-1">
            {[
              {label: 'Your line', value: 'No answer', ok: false},
              {label: 'Competitor', value: 'Booked', ok: true},
            ].map((row, i) => (
              <div key={row.label}>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span
                    ...
                    style={{color: row.ok ? FUNNEL_COLOURS.accent : FUNNEL_COLOURS.goldDeep}}
                  >
                    {row.value}
                  </span>
                </div>
                <div className="h-1.5 rounded-full overflow-hidden" style={{backgroundColor: `${FUNNEL_COLOURS.ink}2C`}}>
                  <motion.div
                    ...
                    style={{backgroundColor: row.ok ? FUNNEL_COLOURS.accent : FUNNEL_COLOURS.goldDeep}}
                    initial={reduce ? false : {width: '0%'}}
                    animate={play || reduce ? {width: row.ok ? '78%' : '22%'} : {width: '0%'}}
      ...
      <Card index="04" title="Three vendors, three kickoffs">
        {({play, reduce}) => (
