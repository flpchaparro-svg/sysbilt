...
export function MediaCleanPainCards() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
      <Card index="01" title="Phone photos uploaded raw">
        {({play, reduce}) => (
          <div className="space-y-2 py-1">
            {['IMG_9012.jpg', 'IMG_9013.jpg', 'IMG_9014.jpg'].map((file, i) => (
              <motion.div
                key={file}
      ...
      <Card index="02" title="Gallery loads everything">
        {({play}) => (
            ...
            <div className="w-full grid grid-cols-3 gap-1.5">
              {[0, 1, 2, 3, 4, 5].map((i) => (
                <div
                  ...
                  style={{backgroundColor: `${FUNNEL_COLOURS.accent}${i < 3 ? '35' : '18'}`}}
                />
              ))}
            </div>
            <motion.span
      ...
      <Card index="03" title="Speed Fix, then banners">
        {({play, reduce}) => (
          <div className="space-y-3 py-1">
            {[
              {label: 'After Speed Fix', value: 'Good', ok: true},
              {label: 'After new banners', value: 'Slipped', ok: false},
            ].map((row, i) => (
              <div key={row.label}>
                ...
                <div className="h-1.5 rounded-full overflow-hidden" style={{backgroundColor: `${FUNNEL_COLOURS.ink}10`}}>
                  <motion.div
                    ...
                    animate={play || reduce ? {width: row.ok ? '86%' : '34%'} : {width: '0%'}}
      ...
      <Card index="04" title="No rule for what to upload">
        {({play, reduce}) => (
