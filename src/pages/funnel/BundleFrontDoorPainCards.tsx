...
export function BundleFrontDoorPainCards() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
      <Card index="01" title="Profile messy">
        {({play, reduce}) => (
          <div className="space-y-2 py-1">
            {['Wrong hours', 'Old photos', 'Weak category'].map((row, i) => (
              <motion.div
                key={row}
      ...
      <Card index="02" title="Reviews thin">
        {({play}) => (
          <div className="flex flex-col items-center justify-center gap-3 py-2">
            <div className="w-full flex items-end justify-center gap-2 h-14">
              {[28, 36, 22, 40, 18].map((h, i) => (
                <motion.div
                  key={i}
                  className="w-5 rounded-sm"
                  style={{backgroundColor: `${FUNNEL_COLOURS.accent}40`, height: h}}
      ...
      <Card index="03" title='Booking is still "call us"'>
        {({play, reduce}) => (
          <div className="space-y-3 py-1">
            {[
              {label: 'Site CTA', value: 'Call us', ok: false},
              {label: 'What they want', value: 'Book now', ok: true},
            ].map((row, i) => (
              <div key={row.label}>
                ...
                <div className="h-1.5 rounded-full overflow-hidden" style={{backgroundColor: `${FUNNEL_COLOURS.ink}10`}}>
                  <motion.div
                    ...
                    animate={play || reduce ? {width: row.ok ? '82%' : '38%'} : {width: '0%'}}
      ...
      <Card index="04" title="Three projects never start">
        {({play, reduce}) => (
