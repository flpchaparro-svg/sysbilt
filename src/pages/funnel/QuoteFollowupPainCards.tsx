...
export function QuoteFollowupPainCards() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
      <Card index="01" title="Quotes sit in Sent">
        {({play, reduce}) => (
          <div className="space-y-2 py-1">
            {['Quote #1801', 'Quote #1814', 'Quote #1830'].map((file, i) => (
              <motion.div
                key={file}
      ...
      <Card index="02" title="Staff hate chasing">
        {({play}) => (
      ...
      <Card index="03" title="Only the big ones get remembered">
        {({play, reduce}) => (
          <div className="space-y-3 py-1">
            {[
              {label: 'Large quote', value: 'Chased', ok: true},
              {label: 'Mid quote', value: 'Forgotten', ok: false},
            ].map((row, i) => (
              <div key={row.label}>
      ...
      <Card index="04" title="No record of nudges">
        {({play, reduce}) => (
