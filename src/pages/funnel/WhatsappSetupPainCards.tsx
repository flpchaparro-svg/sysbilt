...
export function WhatsappSetupPainCards() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
      <Card index="01" title="Owner is the bottleneck">
        {({play, reduce}) => (
          <div className="space-y-2 py-1">
            {['Hours?', 'Parking?', 'Price range?'].map((q, i) => (
              <motion.div
                key={q}
      ...
      <Card index="02" title="No labels, same pile">
        {({play}) => (
          <div className="flex flex-col items-center justify-center gap-3 py-2">
            <div className="w-full space-y-1.5">
              {['New lead', 'Spam promo', 'Booked client'].map((row) => (
                <div
                  key={row}
                  className="rounded-md border px-3 py-2"
                  style={{borderColor: `${FUNNEL_COLOURS.accent}35`, backgroundColor: `${FUNNEL_COLOURS.accent}0A`}}
                >
      ...
      <Card index="03" title="Tone depends on who typed">
        {({play, reduce}) => (
          <div className="space-y-3 py-1">
            {[
              {label: 'Staff A', value: 'Casual'},
              {label: 'Staff B', value: 'Formal'},
            ].map((row, i) => (
              <div key={row.label}>
                ...
                <div className="h-1.5 rounded-full overflow-hidden" style={{backgroundColor: `${FUNNEL_COLOURS.ink}2C`}}>
                  <motion.div
                    ...
                    animate={play || reduce ? {width: i === 0 ? '70%' : '45%'} : {width: '0%'}}
      ...
      <Card index="04" title="Cannot hand off when away">
        {({play, reduce}) => (
