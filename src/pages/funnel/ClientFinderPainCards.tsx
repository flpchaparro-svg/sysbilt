...
export function ClientFinderPainCards() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
      <Card index="01" title="Quiet month, empty dial pad">
        {({play, reduce}) => (
          <div className="space-y-2 py-1">
            {['Referral dried up', 'No shortlist', 'Who do we ring'].map((row, i) => (
              <motion.div
                key={row}
      ...
      <Card index="02" title="Bought lists burn trust">
        {({play}) => (
      ...
      <Card index="03" title="Retainer activity, no fit">
        {({play, reduce}) => (
          <div className="space-y-3 py-1">
            {[
              {label: 'Emails sent', value: 'Busy', ok: false},
              {label: 'Who actually fits', value: 'Unknown', ok: false},
            ].map((row, i) => (
              <div key={row.label}>
                ...
                <div className="h-1.5 rounded-full overflow-hidden" style={{backgroundColor: `${FUNNEL_COLOURS.ink}2C`}}>
                  <motion.div
                    ...
                    animate={play || reduce ? {width: i === 0 ? '92%' : '18%'} : {width: '0%'}}
      ...
      <Card index="04" title="No script, no plan">
        {({play, reduce}) => (
