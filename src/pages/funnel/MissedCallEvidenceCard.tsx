...
export type MissedCallEvidence = TestedEvidence | TryEvidence

/**
 * “This is you, right now” for Missed-Call — logged test call, or self-test fallback.
 */
export function MissedCallEvidenceCard({evidence}: {evidence: MissedCallEvidence}) {
  ...
  const label = isTested
    ...
    : 'TRY IT NOW'
  const body = isTested
    ? `We rang ${evidence.business}. It rang out. No text came back.`
    : 'Grab another phone, ring your own number, and let it ring out. Whatever happens in the next sixty seconds is what every missed caller gets.'
