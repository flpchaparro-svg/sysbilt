...
export function SopPlaybookPainCards() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
      <Card index="01" title="Only one person knows">
        {({play, reduce}) => (
          <div className="space-y-2 py-1">
            {['Expert away', 'Work freestyles', 'Standards slip'].map((label, i) => (
              <motion.div
                key={label}
      ...
      <Card index="02" title="AI use is secret">
        {({play}) => (
          <div className="flex flex-col items-center justify-center gap-3 py-2">
            <div className="w-full grid grid-cols-2 gap-1.5">
              {['Private prompt', 'Different tone', 'No check', 'Guesses'].map((label, i) => (
                <div
                  key={label}
                  className="rounded-sm px-2 py-2 text-center"
                  style={{backgroundColor: `${FUNNEL_COLOURS.accent}${i < 2 ? '28' : '18'}`}}
                >
                  <span className="font-mono text-[8px] font-bold uppercase tracking-wide" style={{color: FUNNEL_COLOURS.accent}}>
                    {label}
                  </span>
      ...
      <Card index="03" title="Onboarding by osmosis">
        {({play, reduce}) => (
          <div className="space-y-3 py-1">
            {[
              {label: 'Shadow the expert', value: 'Slow', ok: false},
              {label: 'Written path', value: 'Missing', ok: false},
            ].map((row, i) => (
              <div key={row.label}>
                ...
                <div className="h-1.5 rounded-full overflow-hidden" style={{backgroundColor: `${FUNNEL_COLOURS.ink}2C`}}>
                  <motion.div
                    ...
                    animate={play || reduce ? {width: i === 0 ? '70%' : '22%'} : {width: '0%'}}
      ...
      <Card index="04" title="Quality swings with the shift">
        {({play, reduce}) => (
          <div className="relative flex flex-col items-center justify-center gap-2 py-2">
            ...
            >
              ...
              <p className="font-sans text-[13px] font-semibold" style={{color: FUNNEL_COLOURS.ink}}>
                Standard follows the person, not the job
              </p>
            </motion.div>
            <motion.p
