...
/** Leak: blocked access beside a cleared critical path. */
export function A11yPassLeakPair() {
    ...
    >
      ...
      >
        ...
        <div className="p-4 md:p-5 flex flex-col justify-center gap-2 min-h-[168px]">
          {['Contrast fails reading', 'Fields missing labels', 'Keyboard stuck in loop'].map((item) => (
            <div
              key={item}
      ...
      >
        ...
        <div className="p-4 md:p-5 flex flex-col justify-center gap-2 min-h-[168px]">
          {['Readable contrast', 'Labels on every field', 'Tab path completes'].map((item, i) => (
            <motion.div
              key={item}
