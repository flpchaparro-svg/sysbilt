...
export function NoshowRescuePainCards() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
      <Card index="01" title="Reminders are manual or missing">
        {({play, reduce}) => (
          <div className="space-y-2 py-1">
            {['Mon 9:00', 'Tue 11:30', 'Wed 14:00'].map((slot, i) => (
              <motion.div
                key={slot}
      ...
      <Card index="02" title="No-shows get a sigh, not a link">
        {({play}) => (
      ...
      <Card index="03" title="Staff scramble last minute">
        {({play, reduce}) => (
          <div className="space-y-3 py-1">
            {[
              {label: 'Filled early', value: 'Calm', ok: true},
              {label: 'Filled late', value: 'Scramble', ok: false},
            ].map((row, i) => (
              <div key={row.label}>
      ...
      <Card index="04" title="Process gap, not bad customers">
        {({play, reduce}) => (
