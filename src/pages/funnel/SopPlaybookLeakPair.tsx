...
/** Leak: hero culture beside a reusable playbook. */
export function SopPlaybookLeakPair() {
    ...
    >
      ...
      >
        ...
        <div className="p-4 md:p-5 flex flex-col justify-center gap-2 min-h-[168px]">
          {['Proper way lives with one person', 'AI use is secret and uneven', 'New staff learn by osmosis'].map((row) => (
            <div
              key={row}
      ...
      >
        ...
        <div className="p-4 md:p-5 flex flex-col justify-center gap-2 min-h-[168px]">
          {['SOP steps written', 'Prompts tied to the job', 'Checks before send'].map((row, i) => (
            <motion.div
              key={row}
