      ...
      <Panel label="The landing">
        <motion.div
          animate={
            reduce || !inView
              ? undefined
              : {opacity: [0.55, 1, 0.55], x: [0, 4, 0]}
          }
          transition={{duration: 2.8, repeat: Infinity, ease: 'easeInOut'}}
          className="space-y-2"
        >
          {['Home', 'About', 'Services', 'Blog', 'Contact', 'Popup'].map((item) => (
            <div
              key={item}
        ...
        <p className="mt-3 font-mono text-[9px] font-bold uppercase tracking-[0.18em]" style={{color: FUNNEL_COLOURS.accent}}>
          Lost cursor
        </p>
      </Panel>
