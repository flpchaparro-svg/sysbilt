...
export default function BseChapterArticlePage() {
  const { chapterSlug } = useParams<{ chapterSlug: string }>()
  const chapter = chapterSlug ? getBseChapterBySlug(chapterSlug) : undefined

  if (!chapter) {
    return <Navigate to={BSE_HUB_PATH} replace />
  }

  const blocks = extractChapterBlocks(chapter.pages)
  const cover = BSE_CHAPTER_COVERS[chapter.num]
  const canonical = `${SITE_ORIGIN}${bseChapterPath(chapter.slug)}`
  const htmlTitle = `${chapter.seoTitle} | SYSBILT`
  const prev = chapter.num > 1 ? getBseChapterByNum(chapter.num - 1) : undefined
  const next = chapter.num < BSE_CHAPTERS.length ? getBseChapterByNum(chapter.num + 1) : undefined
    ...
    <div className="btw-root min-h-screen bg-cream text-dark selection:bg-dark selection:text-cream pt-[100px] md:pt-[140px] pb-16 md:pb-24">
      ...
      <article className="mx-auto w-full max-w-[720px] px-4 md:px-6">
        <nav aria-label="Breadcrumb" className="mb-8 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-dark/45">
          ...
          <Link to={BSE_HUB_PATH} className="hover:text-dark transition-colors">
            {BSE_META.title}
          </Link>
          <span className="mx-2">/</span>
        ...
        <GuideChapterNav prev={prev} next={next} chapterPath={bseChapterPath} />
...
export function isBseChapterSlug(slug: string): boolean {
  if (slug === 'read') return false
  return BSE_CHAPTERS.some((c) => c.slug === slug)
}
