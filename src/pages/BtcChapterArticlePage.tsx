...
import {
  ...
  getBtcChapterByNum,
  pillarLabel,
} from '../built-to-close/chapter-seo'
import { BTC_CHAPTER_COVERS } from '../built-to-close/chapter-covers'
import { chapterCoverWebSrc } from '../guides/chapter-cover-types'
import { BtwFlowList } from '../built-to-work/components/BtwBlocks'
import { BtcPdfCta } from '../built-to-close/components/BtcPdfCta'
...
export default function BtcChapterArticlePage() {
  const { chapterSlug } = useParams<{ chapterSlug: string }>()
  const chapter = chapterSlug ? getBtcChapterBySlug(chapterSlug) : undefined

  if (!chapter) {
    return <Navigate to={BTC_HUB_PATH} replace />
  }

  const blocks = extractChapterBlocks(chapter.pages)
  const cover = BTC_CHAPTER_COVERS[chapter.num]
  const canonical = `${SITE_ORIGIN}${btcChapterPath(chapter.slug)}`
  const htmlTitle = `${chapter.seoTitle} | SYSBILT`
  const prev = chapter.num > 1 ? getBtcChapterByNum(chapter.num - 1) : undefined
  const next = chapter.num < BTC_CHAPTERS.length ? getBtcChapterByNum(chapter.num + 1) : undefined
    ...
    <div className="btw-root min-h-screen bg-cream text-dark selection:bg-dark selection:text-cream pt-[100px] md:pt-[140px] pb-16 md:pb-24">
      ...
      <article className="mx-auto w-full max-w-[720px] px-4 md:px-6">
        <nav aria-label="Breadcrumb" className="mb-8 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-dark/45">
          ...
          <Link to={BTC_HUB_PATH} className="hover:text-dark transition-colors">
            {BTC_META.title}
          </Link>
          <span className="mx-2">/</span>
        ...
        <GuideChapterNav prev={prev} next={next} chapterPath={btcChapterPath} />
...
export function isBtcChapterSlug(slug: string): boolean {
  if (slug === 'read') return false
  return BTC_CHAPTERS.some((c) => c.slug === slug)
}
