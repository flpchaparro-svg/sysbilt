...
import { BTT_BOOK_PATH, BTT_HUB_PATH } from '../chapter-seo'
...
export function BttPdfCta({ variant = 'card', className = '' }: Props) {
  if (variant === 'inline') {
    return (
      <p className={`font-sans text-[15px] leading-relaxed text-dark/70 ${className}`}>
        ...
        >
          Download the polished PDF edition of {BTT_META.title}
        </Link>
        .
      </p>
    )
