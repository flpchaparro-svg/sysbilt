...
import { BTS_BOOK_PATH, BTS_HUB_PATH } from '../chapter-seo'
...
export function BtsPdfCta({ variant = 'card', className = '' }: Props) {
  if (variant === 'inline') {
    return (
      <p className={`font-sans text-[15px] leading-relaxed text-dark/70 ${className}`}>
        ...
        >
          Download the polished PDF edition of {BTS_META.title}
        </Link>
        .
      </p>
    )
