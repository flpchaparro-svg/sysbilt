export type { BtwBlock, BtwPage, BtwPageLayout } from '../built-to-work/types'

export const BTM_META = {
  slug: 'built-to-multiply',
  title: 'Built to Multiply',
  subtitle: 'How one voice becomes a month of content, and why the system matters more than the posts.',
  coverLegend: 'For Australian businesses who want content that earns, not a flood that spins',
  badgeLabel: 'Content Systems',
  badgeLink: '/pillar5',
  seoTitle: 'Built to Multiply: Content Without the Flood',
  seoDescription:
    'How one voice becomes a month of content: strategy, one source, the toolkit, the production line, measurement, and wiring that turns attention into business. Free from SYSBILT.',
} as const
