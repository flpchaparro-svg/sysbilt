...
export function BtmCoverPage() {
  const shareUrl = `https://sysbilt.com/guides/${BTM_META.slug}`
      ...
      <div className="relative flex flex-1 flex-col justify-between p-[clamp(48px,7vw,80px)]">
        <div className="grid grid-cols-[1fr_auto_1fr] items-start gap-4">
          ...
          <div className="font-mono text-[10px] font-bold uppercase tracking-[0.22em] text-right leading-[1.7]" style={{ color: 'rgba(26,26,26,0.55)' }}>
            The SYSBILT<br />Business Guides<br /><span style={{ color: BTW_TOKENS.goldOnCream }}>No. 06</span>
          </div>
        </div>
        <div className="my-auto">
          <div className="font-mono text-[11px] font-bold uppercase tracking-[0.24em] mb-[26px]" style={{ color: BTW_TOKENS.goldOnCream }}>/ A FIELD GUIDE FOR CONTENT</div>
          <h1 className="font-serif font-medium leading-[0.9] tracking-[-0.045em] m-0" style={{ fontSize: 'clamp(56px, 11vw, 104px)', color: BTW_TOKENS.ink }}>
            Built<br />to <span className="italic" style={{ color: BTW_TOKENS.goldOnCream }}>Multiply</span>
          </h1>
          <div className="w-16 h-[2px] my-10" style={{ backgroundColor: BTW_TOKENS.gold }} />
          <p className="font-sans font-light leading-[1.55] max-w-[34ch] m-0" style={{ fontSize: 'clamp(17px, 1.8vw, 21px)', color: 'rgba(26,26,26,0.72)' }}>{BTM_META.subtitle}</p>
        </div>
...
export function BtmClosingPage() {
  return (
      ...
      <div className="relative flex flex-1 flex-col justify-between p-[clamp(48px,7vw,80px)]">
        ...
        <div className="my-auto">
          ...
          <h2 className="font-serif font-medium text-[clamp(38px,7vw,72px)] leading-[0.98] tracking-[-0.035em] text-[#FFF2EC] m-0 max-w-[18ch]">
            A system, not a <span className="italic text-[#D4A84B]">flood</span>
          </h2>
          <p className="font-sans font-light text-[clamp(16px,1.8vw,20px)] leading-[1.6] text-[#FFF2EC]/80 max-w-[50ch] mt-8 mb-10">
            Start with a Content Systems Review. A straight, no-obligation look at whether your content is a system or a flood, and what we would build first. No pitch, no pressure.
          </p>
        ...
        <div className="pt-6 border-t border-[#FFF2EC]/14 flex justify-between flex-wrap gap-3 font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-[#FFF2EC]/50">
          <span>Built to Multiply · No. 06</span>
          <span className="text-[#D4A84B]">Less made. More earned.</span>
        </div>
      </div>
    </div>
  )
