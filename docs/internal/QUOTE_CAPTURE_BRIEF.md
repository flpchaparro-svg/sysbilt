...
**Phase 5 live path (proof install):** **OK 8 Aug 2026 (SMS moved to ClickSend 18 Aug 2026).** `/q/proof-landscapes` and embed `/embed/q/proof-landscapes`. Soft no for out-of-catalogue. On **See my quotation**, submit rebuilds the total server-side, creates Zoho Quote (SYSBILT org for proof), Stripe Checkout (full pay), visitor email (Resend) + SMS (ClickSend shared pool), owner HubSpot note + Slack. Pay only opens the Checkout URL already returned. Local `npm run dev` serves `POST /api/quote-capture/submit` via Vite middleware. Demo `/demo/quote-capture` stays fake. Per-client Zoho OAuth and Stripe Connect stay for the first real client handover.
